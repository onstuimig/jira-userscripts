// ==UserScript==
// @name         JIRA Prevent click edit
// @description  Prevents edititing the issue description on click and adds an `Edit` button to the bottom of the description
// @namespace    https://github.com/onstuimig/jira-userscripts
// @version      1.1.2
// @author       Onstuimig - Jeroen Rothbauer
// @match        https://onstuimig.atlassian.net/*
// @icon         data:image/png;base64,
// @grant        none
// ==/UserScript==

(function() {
    'use strict';

	const observer = new MutationObserver(function(mutations) {
		for (let i = 0; i < mutations.length; i++) {
			const mutation = mutations[i];
			if(mutation.addedNodes.length){
				/*
				let rich_text = document.querySelectorAll('[class^="FieldBaseWrapper-"],[class*=" FieldBaseWrapper-"]')[1];
				let edit_button = document.querySelectorAll('[class^="EditButton-"],[class*=" EditButton-"]')[1];
				let content_holder = document.querySelectorAll('[class^="Content-"],[class*=" Content-"]')[1];
				*/

				let rich_text = document.querySelectorAll('button[aria-label="Edit Description"] + [data-read-view-fit-container-width]')[0];
				let edit_button = document.querySelectorAll('button[aria-label="Edit Description"]')[0];
				let content_holder = rich_text;

				if(rich_text && edit_button && content_holder){
					preventClickEdit(rich_text, edit_button, content_holder);
					observer.disconnect();
					return;
				}
			}
		}
	});
	observer.observe(document.body, {
		childList: true,
		subtree: true
	});

	function preventClickEdit(rich_text, edit_button, content_holder) {

        if(!rich_text) {
            return;
        }

        if(!rich_text.parentElement) {
            return;
        }

        rich_text.parentElement.addEventListener('click', function(e){
			if(!window.allowEdit){
				if(e.target.tagName == 'IMG' || e.target.classList.contains('media-card-blanket') || (e.target.closest('.rich-media-item') !== null)){
					return true;
				}
				e.stopPropagation();
			}
        });

		if(content_holder) {
			content_holder.style = "background-color:#FFFFFF !important;";
		}

		if(edit_button) {
			const editLink = document.createElement('a');
			editLink.href="#edit";
			editLink.innerText = "Edit";
			editLink.style = "margin:8px 8px 0;color:#6B778C;display:inline-block;";
			editLink.addEventListener('click', function(e){
				e.preventDefault();

				window.allowEdit = true;
				edit_button.click();
				edit_button.click();
			});

			rich_text.insertAdjacentElement('afterend', editLink);
		}
	}
})();
